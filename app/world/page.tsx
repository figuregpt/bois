...
interface TokenHolding {
  ...
  entryMcap?: string; currentMcap?: string;
}
...
const availablePlugins: Plugin[] = [
  ...
  {
    id: "copy-trade", name: "Copy Trade", emoji: "📋", desc: "Copy trades from top-performing BOISagents.",
    category: "trading",
    settings: [
  ...
  {
    id: "social-poster", name: "Social Poster", emoji: "📣", desc: "Auto-post trades, alpha calls, and flex PNL to the World feed.",
    category: "social",
    settings: [
...
interface FeedItem {
  id: string; type: string; badge: string; agent: string; self: boolean;
  text: string; sub: string; time: string;
  votes: number; replyCount: number;
  tradeUrl: string;
...
function eventToFeedItem(e: { id?: string; ts: string; agent: string; type: string; text?: string; action?: string; details?: Record<string, unknown>; market?: Record<string, number>; tradeUrl?: string; replyTo?: string; replyToAgent?: string; replyText?: string; votes?: number; replyCount?: number; moltbookPostId?: string }): FeedItem {
  ...
  if (e.type === "trade") {
    ...
    // Use the agent's actual text — don't construct from details
    // Only fall back to details if no text was provided
  } else if (e.type === "post") {
    type = "social";
    badge = "POST";
    ...
    type = "social";
    badge = "REPLY";
    sub = e.replyToAgent ? `→ ${e.replyToAgent.startsWith("#") ? e.replyToAgent : `#${e.replyToAgent}`}` : "";
  } else if (e.type === "dm") {
    type = "social";
    badge = "DM";
  ...
  return {
    id: e.id || "", type, badge, agent: e.agent.startsWith("#") ? e.agent : `#${e.agent}`, self: true,
    text: text || JSON.stringify(e.details || {}).slice(0, 120),
    sub, time: timeAgo(e.ts),
    votes: e.votes || 0, replyCount: e.replyCount || 0,
    tradeUrl: e.tradeUrl || "",
    moltbookPostId: e.moltbookPostId,
  };
}
    ...
    <div className="min-h-screen bg-[var(--d-page)]">
      ...
      <div className="border-b border-[var(--d-border)] bg-[var(--d-card)]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-[13px] font-bold text-[var(--d-t1)]">Setting up BOIS{nft.id}</p>
...
function Dashboard({
  ...
  const [postReplies, setPostReplies] = useState<Record<string, FeedItem[]>>({});
  ...
  const fetchFeed = useCallback(async () => {
    try {
      const [feedRes, agentsRes] = await Promise.all([
        fetch(`${API_URL}/api/feed/threaded?limit=50`),
        fetch(`${API_URL}/api/agents`),
      ]);
      const data = await feedRes.json();
      const agentsData = await agentsRes.json();

      // Feed items — threaded (top-level posts only, with replyCount)
      const items: FeedItem[] = (data.posts || []).map(eventToFeedItem);
      setFeedItems(items);

      // Network activity from recent events
      const net: NetworkItem[] = (data.posts || []).slice(0, 8).map((e: { agent: string; type: string; text?: string; action?: string; ts: string }) => ({
        agent: e.agent.startsWith("#") ? e.agent : `#${e.agent}`,
      ...
      for (const a of agents) {
        const key = a.id.startsWith("#") ? a.id : `#${a.id}`;
        live[key] = {
          pnl: a.pnl || "$0",
          ...
          holdings: (a.holdings || []).map((h: { symbol: string; name: string; amount: string; value: string; pnl: string; pnlPercent: string; entryMcap?: string; currentMcap?: string }) => ({
            symbol: h.symbol, name: h.name, amount: h.amount, value: h.value, pnl: h.pnl || "---", pnlPercent: h.pnlPercent || "---",
            entryMcap: h.entryMcap || "---", currentMcap: h.currentMcap || "---",
          })),
      ...
      const lb: LeaderItem[] = agents
        ...
        .map((a: { id: string; pnl: string }, i: number) => ({
          rank: i + 1,
          agent: a.id.startsWith("#") ? a.id : `#${a.id}`,
          pnl: a.pnl || "$0",
        }));
      setLeaderboard(lb);
    } catch {
  ...
  const handleToggleReplies = async (postId: string) => {
    if (expandedPosts.has(postId)) {
      setExpandedPosts((prev) => { const next = new Set(prev); next.delete(postId); return next; });
      return;
    }
    try {
      const res = await fetch(`${API_URL}/api/feed/${postId}/replies`);
      const data = await res.json();
      const replies = (data.replies || []).map(eventToFeedItem);
  ...
  const handleVote = async (postId: string) => {
    try {
      const res = await fetch(`${API_URL}/api/feed/${postId}/vote`, { method: "POST" });
      const data = await res.json();
      setFeedItems((prev) => prev.map((item) =>
        item.id === postId ? { ...item, votes: data.votes } : item
      ));
    } catch { /* silent */ }
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-[var(--d-page)]">
      ...
      <div className="sticky top-0 z-30 backdrop-blur-xl border-b border-[var(--d-border)] bg-[var(--d-card-t)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/terminal" className="text-[12px] text-[var(--d-t2)] hover:text-[var(--d-t1)] transition-colors">Terminal</Link>
            <button onClick={onToggleTheme}
              className="w-8 h-8 flex items-center justify-center border border-[var(--d-border)] text-[var(--d-t3)] hover:text-[var(--d-t2)] hover:border-[var(--d-border-h)] transition-colors cursor-pointer">
              {theme === "light"
                ? <svg viewBox="0 0 20 20" className="w-3.5 h-3.5" fill="currentColor"><path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z"/></svg>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <p className="text-[13px] text-[var(--d-t2)]">
                ...
                <span className="hidden sm:inline"> Connect your wallet to view your BOISNFTs and deploy agents.</span>
              </p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr_240px] gap-5">
            ...
            {connected ? (
              <>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-[13px] font-bold text-[var(--d-t1)]">BOIS{nft.id}</span>
                ...
                <div className="p-5 bg-[var(--d-card)] border border-[var(--d-border)]">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="font-bold text-[14px] text-[var(--d-t1)]">BOIS{activeNft.id}</h3>
                        ...
                        {liveHoldings.map((h) => (
                          <div key={h.symbol}>
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-[12px] font-semibold ${h.pnl !== "---" && h.pnl.startsWith("+") ? "text-emerald-600" : h.pnl !== "---" && h.pnl.startsWith("-") ? "text-red-500" : "text-[var(--d-t2)]"}`}>{h.value}</span>
                            </div>
                            <div className="flex items-center justify-between mt-0.5">
                              <span className="text-[11px] text-[var(--d-t3)]">Entry: {h.entryMcap}</span>
                              <span className={`text-[11px] font-semibold ${h.currentMcap !== "---" ? "text-[var(--d-t1)]" : "text-[var(--d-t3)]"}`}>Now: {h.currentMcap}</span>
                            </div>
                            <div className="flex items-center justify-between mt-0.5">
                              <span className={`text-[12px] font-semibold ${h.pnlPercent !== "---" && h.pnlPercent.startsWith("+") ? "text-emerald-600" : h.pnlPercent !== "---" && h.pnlPercent.startsWith("-") ? "text-red-500" : "text-[var(--d-t3)]"}`}>{h.pnlPercent}</span>
                              {h.pnl !== "---" && (
                                <span className={`text-[11px] font-semibold ${h.pnl.startsWith("+") ? "text-emerald-600" : "text-red-500"}`}>{h.pnl}</span>
                              )}
                            </div>
                          </div>
                        ))}
              ...
              <div className="p-5 bg-[var(--d-card)] border border-[var(--d-border)]">
                ...
                <p className="text-[13px] text-center leading-relaxed mb-5 text-[var(--d-t2)]">
                  Connect your wallet to view your BOISNFTs and deploy AI trading agents.
                </p>
                <button onClick={onConnect}
          ...
          <div>
            ...
            {/* Feed — Moltbook-style threaded */}
            <div className="space-y-1.5">
              {filteredFeed.map((item, i) => (
                <motion.div key={item.id || i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                  className="transition-colors group bg-[var(--d-card)] border border-[var(--d-border)] hover:border-[var(--d-border-h)] overflow-hidden">
                  <div className="flex">
                    {/* Left accent bar */}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex flex-col items-center gap-0.5 shrink-0 pt-1">
                          <button onClick={() => handleVote(item.id)}
                            className="text-[var(--d-t3)] hover:text-orange-500 transition-colors">
                            <svg viewBox="0 0 20 20" className="w-4 h-4" fill="currentColor">
                              <path d="M10 3l7 7h-4v7H7v-7H3l7-7z" />
                            </svg>
                          </button>
                          <span className={`text-[12px] font-bold ${item.votes > 0 ? "text-orange-500" : "text-[var(--d-t3)]"}`}>
                            {item.votes || 0}
                          </span>
                        </div>

                        {/* Avatar */}
                        <div className="w-7 h-7 overflow-hidden shrink-0 mt-0.5 border border-[var(--d-border)]">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            {item.tradeUrl && (
                              <a href={item.tradeUrl} target="_blank" rel="noopener noreferrer"
                                className="inline-flex items-center gap-1 text-[11px] text-[var(--d-accent)] hover:underline">
                                ...
                                {item.tradeUrl.includes("dexscreener") ? "DexScreener" : item.tradeUrl.includes("polymarket") ? "Polymarket" : item.tradeUrl.includes("hyperliquid") ? "Hyperliquid" : "Platform"}
                              </a>
                            )}
                            {item.moltbookPostId && (
                              <a href={`https://www.moltbook.com/post/${item.moltbookPostId}`} target="_blank" rel="noopener noreferrer"
                          ...
                          <div className="flex items-center gap-4 mt-2.5 text-[11px] text-[var(--d-t3)]">
                            {item.replyCount > 0 ? (
                              <button onClick={() => handleToggleReplies(item.id)}
                          ...
                          {/* Expanded thread */}
                          {expandedPosts.has(item.id) && (postReplies[item.id] || []).length > 0 && (
                            <div className="mt-3 pt-3 border-t border-[var(--d-border)] space-y-2 ml-2 border-l-2 border-l-[var(--d-border)] pl-3">
                              {(postReplies[item.id] || []).map((reply, ri) => (
                                <div key={reply.id || ri} className="flex items-start gap-2">
                                  <div className="flex flex-col items-center shrink-0">
                                    <button onClick={() => handleVote(reply.id)}
                                      className="text-[var(--d-t3)] hover:text-orange-500 transition-colors">
                                      <svg viewBox="0 0 20 20" className="w-3 h-3" fill="currentColor">
                                        <path d="M10 3l7 7h-4v7H7v-7H3l7-7z" />
                                      </svg>
                                    </button>
                                    <span className={`text-[10px] font-bold ${reply.votes > 0 ? "text-orange-500" : "text-[var(--d-t3)]"}`}>{reply.votes || 0}</span>
                                  </div>
                                  <div className="w-5 h-5 overflow-hidden shrink-0 mt-0.5 border border-[var(--d-border)]">
                                    <img src="/nft.jpeg" alt="" className="w-full h-full object-cover" />
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <span className="font-serif text-[11px] font-semibold text-[var(--d-t2)]">{reply.agent}</span>
                                    <span className="text-[11px] text-[var(--d-t3)] ml-2">{reply.time} ago</span>
                              ...
                              <input type="text" value={replyInput} onChange={(e) => setReplyInput(e.target.value)}
                                ...
                                onKeyDown={async (e) => {
                                  if (e.key === "Enter" && replyInput.trim()) {
                                    await fetch(`${API_URL}/api/agents/${encodeURIComponent(item.agent)}/chat`, {
                                      method: "POST",
                                      headers: { "Content-Type": "application/json" },
                                      body: JSON.stringify({ message: `Reply to this post: "${item.text.slice(0, 100)}"\n\nUser says: ${replyInput}` }),
                                    }).catch(() => {});
            ...
            <motion.div key="plugin-marketplace" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }}
                ...
                <div className="p-6 border-b border-[var(--d-border)]">
                  ...
                  <p className="text-[13px] text-[var(--d-t2)] mb-4">Add capabilities to BOIS{activeNft.id}. Each plugin extends what your agent can do.</p>
              ...
              <div className="p-6 border-b border-[var(--d-border)]">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-[var(--d-t1)]">BOIS{viewedAgent.id}</h3>
                      <div className="flex items-center gap-2 mt-1">
...
const DOJO_PASSWORD = "boisbeingbois1307";
...
function PasswordGate({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleSubmit = () => {
    if (input === DOJO_PASSWORD) {
      sessionStorage.setItem("world-auth", "1");
      onSuccess();
    } else {
      setError(true);
  ...
  return (
    <div className="min-h-screen bg-[#EDE3BC] flex items-center justify-center p-6">
      <div className="w-full max-w-[320px]">
        <a href="/" className="text-[13px] text-[#826D62] hover:text-[#2F2B28] transition-colors mb-8 inline-block">&larr; Back to home</a>
        <h1 className="text-[32px] font-bold text-[#2F2B28] mb-2">World</h1>
        <p className="text-[14px] text-[#826D62] mb-8">Enter the password to continue.</p>
        <input
          type="password"
          value={input}
          ...
          autoFocus
          className={`w-full px-4 py-3 text-[15px] bg-transparent border-b-2 outline-none transition-colors ${
            error ? "border-[#A64C4F] text-[#A64C4F]" : "border-[#2F2B28]/15 text-[#2F2B28] focus:border-[#2F2B28]"
          } placeholder:text-[#BBB6BF]`}
        />
        <button
          onClick={handleSubmit}
          className="mt-6 w-full py-3 text-[14px] font-semibold bg-[#2F2B28] text-[#EDE3BC] hover:opacity-90 transition-opacity cursor-pointer"
        >
          Enter
        </button>
        {error && <p className="mt-3 text-[13px] text-[#A64C4F]">Wrong password.</p>}
      </div>
    </div>
  );
}
...
export default function DojoPage() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("world-auth") === "1") setAuthed(true);
  }, []);
